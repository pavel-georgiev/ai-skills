...
8. Enable auto-merge on the MR: `glab mr merge --auto-merge --remove-source-branch`
   - This sets the MR to merge automatically when the pipeline passes
   - If auto-merge fails (e.g., pipeline not configured), inform the user but do not treat it as a fatal error
