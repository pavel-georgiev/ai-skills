...
8. Enable auto-merge on the MR using the GitLab API:
   ...
   - Call: `glab api --method PUT "projects/:fullpath/merge_requests/<MR_IID>/merge" -f auto_merge=true -f should_remove_source_branch=true`
   - Note: Uses `auto_merge=true` (not the deprecated `merge_when_pipeline_succeeds`). This is the **merge endpoint** (`/merge`), not the MR update endpoint.
